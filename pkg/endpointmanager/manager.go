...
func (mgr *EndpointManager) markAndSweep(ctx context.Context) error {
	...
	// TODO: Handle 'ctx' via DeleteEndpoint path
	return mgr.sweepEndpoints(toSweep)
}
...
func (mgr *EndpointManager) sweepEndpoints(markedEndpoints []uint16) error {
	...
	for _, ep := range toSweep {
		...
		errors += mgr.checker.DeleteEndpoint(ep)
	}

	if errors > 0 {
		return fmt.Errorf("%d errors occurred during endpoint garbage collection", errors)
	}
	return nil
}
